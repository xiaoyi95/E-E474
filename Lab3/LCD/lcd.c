...
/* lcd.c contains kernel space code to print commands to an lcd screen through
 * a shift register. The first bit shifted into the register will output at QH,
 * while QH is connected to DB7 on the LCD.
 * Characters from the kernel buffer will print to the screen. When the cursor 
 * reaches the end of the screen (past 16 characters) the screen will shift left
 * to follow the cursor. When it reaches the end of the line (40 characters) the
 * screen will shift back to the front (cursor will be on second line). After
 * all 80 characters have been used up, the display will clear.
 * The user can go to the next line by type "\n", or clear the display by typing
 * "\c". The backslash command will not print to the screen.
 * The writing is delayed after every character to the user can read the screen
 * as it prints.
 */

#include "lcd.h"  // Include the header file for the lcd kernel
...
static int __init driver_entry(void) {
  ...
  if (gpio_request_array(pins, 5)) {
    printk(KERN_ALERT "lcd: request array did not work");
    return 1;
  }
...
static void initial(void) {
  ...
  send(don);    // Display on: blinking cursor
...
static void lcdwrite(int size) {
  ...
  while (count < size) {
    ...
    msleep(150);  // delay so we can read the screen while it is shifting
    count++;
  }
